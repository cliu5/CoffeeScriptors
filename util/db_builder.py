...
def users(): #creates the users db
    ...
    command = "CREATE TABLE users(username TEXT, password TEXT, gold INTEGER, INTEGER PRIMARY KEY (ID))" #user table
    c.execute(command)
    command = "CREATE TABLE todo(username TEXT, difficulty INTEGER, task TEXT)" #to-do list table
    c.execute(command)
    command = "CREATE TABLE pokemon(username TEXT, task TEXT)" #pokemon table
    c.execute(command)
